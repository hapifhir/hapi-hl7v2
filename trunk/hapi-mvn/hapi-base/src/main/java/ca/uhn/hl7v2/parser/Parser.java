 ...
 * objects into HL7 message Strings.
 * 
...
public abstract class Parser {
	...
	/**
	 * Creates a new parser, using the {@link ModelClassFactory}, the
	 * {@link ParserConfiguration} and the {@link ValidationContext} as defined
	 * in the context.
	 * 
	 * @param context
	 */
	...
	/**
	 * @return the set of validation rules that is applied to messages parsed or
	 *         encoded by this parser. Note that this method may return
	 *         <code>null</code>
	 */
	...
	/**
	 * @param theContext
	 *            the set of validation rules to be applied to messages parsed
	 ...
	 *         configuration instructions relating to how a parser should be
	 *         parsing or encoding messages it deals with.
	 */
	...
	/**
	 * Sets the parser configuration for this parser (may not be null). This is
	 * a bean which contains configuration instructions relating to how a parser
	 * should be parsing or encoding messages it deals with.
	 * 
	 * @param theParserConfiguration
	 *            The parser configuration
	 */
	public void setParserConfiguration(
			ParserConfiguration theParserConfiguration) {
		if (theParserConfiguration == null) {
			throw new NullPointerException(
					"theParserConfiguration may not be null");
		}
	 ...
	 * using any other encoding than the one returned. Returns null if the
	 ...
	 *            a String that contains an HL7 message
	 * @param version
	 *            the name of the HL7 version to which the message belongs (eg
	 *            "2.5")
	 * @return a HAPI Message object parsed from the given String
	 * @throws HL7Exception
	 *             if the message is not correctly formatted.
	 ...
	 * 
	 * @param source
	 *            a Message object from which to construct an encoded message
	 *            string
	 * @param encoding
	 *            the name of the HL7 encoding to use (eg "XML"; most
	 *            implementations support only one encoding)
	...
	/**
	 * Called by encode(Message, String) to perform implementation-specific
	 * encoding work.
	 * 
	 * @param source
	 *            a Message object from which to construct an encoded message
	 ...
	 *            string
	 * @param encoding
	 *            the name of the encoding to use (eg "XML"; most
	 *            implementations support only one encoding)
	 * @return the encoded message
	 * @throws HL7Exception
	 *             if the data fields in the message do not permit encoding
	...
	/**
	 * Called by encode(Message) to perform implementation-specific encoding
	 * work.
	 * 
	 * @param source
	 *            a Message object from which to construct an encoded message
	 ...
	 * hopefully avoiding the condition that caused the original error.
	 * </p>
	 * 
	 * @return an MSH segment
	 */
	public abstract Segment getCriticalResponseData(String message)
			throws HL7Exception;
	 ...
	 * correct thread. Implementers of Parsers should take care to make the
	 * implementation of this method very fast and robust. Returns null if MSA-2
	 * can not be found (e.g. if the message is not a response message).
	 */
	 ...
	 * @since 1.0
	 */
	public abstract String doEncode(Segment structure,
	 ...
	 * @since 1.0
	 */
	public abstract String doEncode(Type type,
			EncodingCharacters encodingCharacters) throws HL7Exception;
	 ...
	 * @since 1.0
	 */
	public abstract void parse(Type type, String string,
			EncodingCharacters encodingCharacters) throws HL7Exception;

	/**
	 * Parse a message using a specific model package instead of the default,
	 * using
	 * {@link ModelClassFactory#getMessageClassInASpecificPackage(String, String, boolean, String)}
	 * .
	 * 
	 * <b>WARNING: This method is only implemented in some parser
	 * implementations</b>. Currently it will only work with the PipeParser
	 * parser implementation. Use with caution.
	 */
	public Message parseForSpecificPackage(String message, String packageName)
			throws HL7Exception, EncodingNotSupportedException {
		String encoding = getEncoding(message);
		if (!supportsEncoding(encoding)) {
			throw new EncodingNotSupportedException(
	 ...
	 */
	protected abstract Message doParseForSpecificPackage(String message,
			String version, String packageName) throws HL7Exception,
			EncodingNotSupportedException;

	/**
	 * Instantiate a message type using a specific package name
	 * 
	 * @see ModelClassFactory#getMessageClassInASpecificPackage(String, String,
	 *      boolean, String)
	 */
	 ...
	 *             If there is a problem encoding
	 */
	public abstract void parse(Segment segment, String string,
			EncodingCharacters encodingCharacters) throws HL7Exception;
	 ...
	 * @since 1.0
	 */
	public abstract void parse(Message message, String string)
			throws HL7Exception;

	/**
	 * Creates a version-specific MSH object and returns it as a
	 * version-independent MSH interface. throws HL7Exception if there is a
	 * problem, e.g. invalid version, code not available for given version.
	 */
	public static Segment makeControlMSH(String version,
		...
		try {
			...
			Class<? extends Segment> c = factory
					.getSegmentClass("MSH", version);
			Constructor<? extends Segment> constructor = c
	...
	/**
	 * Returns true if the given string represents a valid 2.x version. Valid
	 * versions include "2.1", "2.2", "2.3", "2.3.1", "2.4", "2.5", "2.6"
	 */
	 ...
	 * is needed because some events share message structures, although it is
	 * not needed when the message structure is explicitly valued in MSH-9-3. If
	 * no mapping is found, returns the original name.
	 * 
	 * @throws HL7Exception
	...
	/**
	 * Returns a copy of the message structure map for a specific version. Each
	 * key is a message type (e.g. ADT_A04) and each value is the corresponding
	 * structure (e.g. ADT_A01).
	 * 
	 * @throws IOException
	 *             If the event map can't be loaded
	 */
	public static Properties getMessageStructures(String version)
			throws IOException {
	 ...
	 */
	private synchronized static Map<String, Properties> getMessageStructures()
			throws IOException {
	...
	private static Map<String, Properties> loadMessageStructures()
			throws IOException {
		Map<String, Properties> map = new HashMap<String, Properties>();
		for (Version v : Version.values()) {
			String resource = "ca/uhn/hl7v2/parser/eventmap/" + v.getVersion()
					+ ".properties";
	...
	/**
	 * Note that the validation context of the resulting message is set to this
	 * parser's validation context. The validation context is used within
	 * Primitive.setValue().
	 * 
	 * @param name
	 *            name of the desired structure in the form XXX_YYY
	 * @param version
	 *            HL7 version (e.g. "2.3")
	 * @param isExplicit
	 *            true if the structure was specified explicitly in MSH-9-3,
	 *            false if it was inferred from MSH-9-1 and MSH-9-2. If false, a
	 *            lookup may be performed to find an alternate structure
	 *            corresponding to that message type and event.
	 * @return a Message instance
	 * @throws HL7Exception
	 *             if the version is not recognized or no appropriate class can
