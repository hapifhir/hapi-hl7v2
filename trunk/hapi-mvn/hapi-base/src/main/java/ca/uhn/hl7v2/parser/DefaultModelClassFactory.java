...
public class DefaultModelClassFactory implements ModelClassFactory
{
    ...
    /**
     * Reloads all message definitions. Note that this is a <b>potentially unsafe operation</b>.
     * Care must be taken to not call this method if there is any chance that HAPI components are
     * currently being used. Ie, this method may be called only before any actual HAPI classes are
     * instantiated. Otherwise, unpredictable behaviour may occur.
     * 
     * @throws Error
     */
    public static void reloadPackages() throws Error {
    ...
    /**
     * <p>
     * Attempts to return the message class corresponding to the given name, by searching through
     ...
     * they appear in unexpected locations (e.g. by local extension) -- see findSegmentClass().
     * </p>
     * <p>
     * Note: the current implementation will be slow if there are multiple user- defined packages,
     * because the JVM will try to load a number of non-existent classes every parse. This should be
     * changed so that specific classes, rather than packages, are registered by name.
     * </p>
     * 
     * @param name
    ...
    /**
     * @see ca.uhn.hl7v2.parser.ModelClassFactory#getSegmentClass(java.lang.String,
     *      java.lang.String)
     */
    ...
    /**
     * <p>
     * Lists all the packages (user-definable) where classes for standard and custom messages may be
     * found. Each package has subpackages called "message", "group", "segment", and "datatype" in
     * which classes for these message elements can be found.
     * </p>
     * <p>
     * At a minimum, this method returns the standard package for the given version. For example,
     * for version 2.4, the package list contains <code>
     * ca.uhn.hl7v2.model.v24</code>. In
     * addition, user-defined packages may be specified for custom messages.
     * </p>
     * <p>
     * If you define custom message classes, and want Parsers to be able to find them, you must
     * register them as follows (otherwise you will get an exception when the corresponding messages
     * are parsed). For each HL7 version you want to support, you must put a text file on your
     * classpath, under the folder /custom_packages, named after the version. For example, for
     * version 2.4, you might put the file "custom_packages/2.4" in your application JAR. Each line
     * in the file should name a package to search for message classes of that version. For example,
     * if you work at foo.org, you might create a v2.4 message structure called "ZFO" and define it
     * in the class
    ...
    /**
     * Returns a package list for the given version, including the standard package for that version
     * and also user-defined packages (see packageList()).
     */
    private static String[] loadPackages(String version) throws HL7Exception {
        ...
        String customPackagesResourceName = MessageFormat.format(CUSTOM_PACKAGES_RESOURCE_NAME_TEMPLATE,
                new Object[]{ version });
     ...
     * 
     * @param name
     *            the segment or message structure name
     * @param version
     *            the HL7 version
     * @param type
     *            'message', 'group', 'segment', or 'datatype'
     */
    private static Class findClass(String name, String version, String type) throws HL7Exception {
        if (Parser.validVersion(version) == false) {
            throw new HL7Exception("The HL7 version " + version + " is not recognized",
        ...
        if (types.indexOf(type) < 0)
            throw new HL7Exception("Can't find " + name + " for version " + version + " -- type must be " + types
                    + " but is " + type);
     ...
     * "ca/uhn/hl7v2/model/v24/". This package should have the packages datatype, segment, group,
     * and message under it. The path ends in with a slash.
     */
    public static String getVersionPackagePath(String ver) throws HL7Exception {
        if (Parser.validVersion(ver) == false) {
            throw new HL7Exception("The HL7 version " + ver + " is not recognized",
                    HL7Exception.UNSUPPORTED_VERSION_ID);
        }
     ...
     * "ca.uhn.hl7v2.model.v24.". This method is identical to
     * <code>getVersionPackagePath(...)</code> except that path separators are replaced with dots.
     */
