...
public class TwoPortConnectionHubTest {
	...
	private static final Logger ourLog = LoggerFactory
			.getLogger(TwoPortConnectionHubTest.class);
	...
	public void testAttach() throws Exception {
		...
		Connection i1 = TestUtils.acquireClientConnection(pipeParser, port1, port2);
		Connection i1again = TestUtils.acquireClientConnection(pipeParser, port1, port2);
		...
		Connection i1OnceMore = hub.attach("localhost", port1, port2, pipeParser,
	...
	public void testAttachToExistingServer() throws Exception {
		...
		Callable<Connection> t = new Callable<Connection>() {
			public Connection call() {
				try {
					return hub.attach("localhost", port1, port2, pipeParser,
							MinLowerLayerProtocol.class);
	...
	public void testAttachToNonexistingServer() throws Exception {
		...
		try {
			hub.attach("localhost", RandomServerPortProvider.findFreePort(), 
					RandomServerPortProvider.findFreePort(), pipeParser,
					MinLowerLayerProtocol.class);
