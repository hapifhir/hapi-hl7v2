...
public class OutboundConnection extends AbstractConnection implements IDestroyable {
	...
	public void sendMessages(List<AbstractMessage<?>> theMessages) {
		new MessageSenderThread(theMessages).start();
	}
	...
	private class ConnectionMonitorThread extends Thread {
		...
		private void doRun() {
			LowerLayerProtocol llpClass = createLlp();
	...
	public class MessageSenderThread extends Thread {
		...
		public MessageSenderThread(List<AbstractMessage<?>> theMessages) {
			myMessages = theMessages;
		}

		@Override
		public void run() {
			StatusEnum initialStatus = getStatus();
			if (initialStatus != StatusEnum.STARTED) {
			...
			for (AbstractMessage<?> abstractMessage : myMessages) {
				i++;

				if (abstractMessage instanceof Hl7V2MessageBase) {

					Message msg = ((Hl7V2MessageBase) abstractMessage).getParsedMessage();
					ourLog.info("Sending message " + i + "/" + myMessages.size() + " of type " + msg.getClass());
					try {
			...
			addActivity(new ActivityInfo(new Date(), "Sent " + i + " message" + (i != 1 ? "s" : "") + " in " + delay + "ms"));
