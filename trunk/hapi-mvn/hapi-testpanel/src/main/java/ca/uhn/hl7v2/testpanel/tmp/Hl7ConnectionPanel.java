...
public class Hl7ConnectionPanel extends JPanel implements IDestroyable {
	...
	private void updateStatus() {
		...
		myStartButton.setEnabled(changesAllowed);
		myStopButton.setEnabled(!changesAllowed);
	...
	public Hl7ConnectionPanel() {
		...
		myNameBox.getDocument().addDocumentListener(new SimpleDocumentListener() {
			...
			public void update(DocumentEvent theE) {
				myConnection.setNameExplicitly(myNameBox.getName());
			}
		});
		
		myRememberAsCheckBox = new JCheckBox("Remember As:");
