...
public enum Version {
    ...
    private static List<Version> ourVersionsOnClasspath;
    ...
    public String modelPackageName() {
        ...
        return String.format("%s.model.%s.", p, getPackageVersion());
    }
    ...
    public static synchronized List<Version> availableVersions() {
        if (ourVersionsOnClasspath == null) {
            ourVersionsOnClasspath = Arrays.stream(values())
    ...
    public static Version lowestAvailableVersion() {
        ...
        return !availableVersions.isEmpty() ? availableVersions.get(0) : null;
    }
     ...
     * on the classes. For instance, if <code>hapi-structures-v24-[version].jar</code>
     * is the only structure JAR on the current JVM classpath, {@link Version#V25} will
     * be returned.
     * <p>
     * </>
    ...
    public static Version highestAvailableVersionOrDefault() {
        List<Version> availableVersions = availableVersions();
        return !availableVersions.isEmpty() ? availableVersions.get(availableVersions.size() - 1) : Version.V25;
    }
