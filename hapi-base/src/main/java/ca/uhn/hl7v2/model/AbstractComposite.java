    ...
    public boolean accept(MessageVisitor visitor, Location location) throws HL7Exception {
        if (visitor.start(this, location)) {
            ...
            for (int i = 0; i < types.length; i++) {
                ...
                // to deal adequately with "subsubcomponents", consider only the first one in any subcomponent.
                if (!t.accept(visitor, nextLocation) || nextLocation.getSubsubcomponent() != -1) {
                    break;
                }
            }
            ...
            for (int i = 0; i < ec.numComponents(); i++) {
                ...
                if (!v.accept(visitor, nextLocation)) {
                    break;
                }
            }
        }
